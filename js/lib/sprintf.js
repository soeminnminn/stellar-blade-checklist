...
export function sprintf(key, ...args) {
  ...
  return sprintf_format(sprintf_parse(key), [key].concat(args));
}

/**
 * Same as {@link sprintf} except it takes an array of arguments, rather than a variable number of arguments
 * @param {string} fmt 
...
function sprintf_format(parse_tree, argv) {
  ...
  for (let i = 0; i < tree_length; i++) {
    if (typeof parse_tree[i] === 'string') {
      ...
      const ph = parse_tree[i]; // convenience purposes only
